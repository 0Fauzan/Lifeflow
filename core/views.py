    ...
    if request.method == 'POST':
        ...
        if action == 'login':
            ...
            if donor_form.is_valid():
                donor_form.save()
                messages.success(request, '✅ Donor account created! You can now login.')
                return redirect('login')
...
def admin_dashboard(request):
    ...
    for n in notifications: n.is_read = True; n.save()
    ...
    context = {
        ...
        'inventory':       inventory,
        'recent_requests': BloodRequest.objects.select_related('requester__hospital_profile').order_by('-request_date')[:8],
...
def admin_donations(request):
    if 'approve' in request.GET:
        ...
        if donation.status == 'pending':
            with transaction.atomic():
                ...
                donation.save()
                inv, _ = BloodInventory.objects.get_or_create(blood_group=donation.blood_group, defaults={'units_available': 0})
    ...
    if 'reject' in request.GET:
        ...
        donation.status = 'rejected'; donation.approved_by = request.user; donation.save()
    ...
    if 'fulfill' in request.GET:
        ...
        if req.status == 'pending':
            ...
            if inv and inv.units_available >= float(req.units_needed):
                ...
                messages.error(request, f'❌ Insufficient stock. Available: {available} units, Needed: {req.units_needed}.')
        return redirect('admin_requests')

    if 'reject' in request.GET:
        req = get_object_or_404(BloodRequest, pk=request.GET['reject'])
        req.status = 'rejected'; req.handled_by = request.user; req.save()
        messages.error(request, '❌ Request rejected.')
        return redirect('admin_requests')

    status_filter = request.GET.get('s', '')
    urgency_order = {'critical': 0, 'high': 1, 'medium': 2, 'low': 3}
    qs = BloodRequest.objects.select_related('requester__hospital_profile')
    if status_filter: qs = qs.filter(status=status_filter)
    qs = sorted(qs, key=lambda r: (urgency_order.get(r.urgency, 9), -r.request_date.timestamp()))
...
def admin_inventory(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if op == 'add': inv.units_available += units
            else:           inv.units_available  = units
            inv.save()
            messages.success(request, f'✅ Inventory updated for {bg}.')
...
def admin_donors(request):
    ...
    if bg_filter:   qs = qs.filter(blood_group=bg_filter)
    if city_filter: qs = qs.filter(user__city=city_filter)
    if search:      qs = qs.filter(Q(user__first_name__icontains=search) | Q(user__last_name__icontains=search) | Q(user__email__icontains=search))
    cities = User.objects.filter(role='donor').values_list('city', flat=True).distinct()
    return render(request, 'admin_panel/donors.html', {
        'donors': qs, 'bg_filter': bg_filter, 'city_filter': city_filter,
...
def admin_camps(request):
    ...
    if 'status' in request.GET and 'id' in request.GET:
        camp = get_object_or_404(Camp, pk=request.GET['id'])
        camp.status = request.GET['status']
        camp.save()
        messages.success(request, '✅ Camp status updated.')
        return redirect('admin_camps')

    return render(request, 'admin_panel/camps.html', {
...
def admin_reports(request):
    ...
    monthly = (
        Donation.objects.filter(status='approved', donation_date__gte=six_months_ago)
        .extra(select={'month': "DATE_FORMAT(donation_date, '%%b %%Y')", 'yr': "YEAR(donation_date)", 'mo': "MONTH(donation_date)"})
        .values('month', 'yr', 'mo')
...
def donor_camps(request):
    ...
    if 'register' in request.GET:
        ...
        if camp.registered_count < camp.capacity:
            ...
            if created:
                camp.registered_count += 1; camp.save()
...
def donor_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if weight:
                donor.weight_kg = weight; donor.save()
